        ...
        for i in range(len(train_ds_list)):
            ...
            with tqdm(train_dl) as tepoch:
                ...
                for i, inputs in enumerate(tepoch):
                    optimizer.zero_grad()
                    tepoch.set_description(f"Train Epoch {epoch+1}/{props.epochs}")
                    tms, tms_pred, pair_tms_hist, capacities, opt = inputs
        
                    if not props.dynamic:
                        capacities = capacities[:1]

                    tms = tms.to(device, dtype=props.dtype)
                    tms_pred = tms_pred.to(device, dtype=props.dtype)
                    pair_tms_hist = pair_tms_hist.to(device, dtype=props.dtype)
                    capacities = capacities.to(device, dtype=props.dtype)
                    opt = opt.to(device, dtype=props.dtype)

                    preds = model(capacities, props, tms, tms_pred, pair_tms_hist, train_dataset.paths_to_edges, props.num_for_loops)
                ...
                end_time = time()
                # count how many data can be processed in 1 second
            ...
            with torch.no_grad():
                with tqdm(val_dl) as vepoch:
                    ...
                    for i, inputs in enumerate(vepoch):
                        ...
                        capacities = capacities.to(device, dtype=props.dtype)
                        opt = opt.to(device, dtype=props.dtype)
                        preds = model(capacities, props, tms, tms_pred, pair_tms_hist, paths_to_edges, props.num_for_loops)
                        loss, loss_val = loss_mlu(preds, opt)
                        loss_sum += loss_val
                        loss_count += 1
    ...
    with torch.no_grad():
        with tqdm(test_dl) as tests:
            ...
            for i, inputs in enumerate(tests):
                ...
                preds = model(capacities, props, tms, tms_pred, pair_tms_hist, test_dataset.paths_to_edges, props.num_for_loops)
                loss, loss_val = loss_mlu(preds, opt)
