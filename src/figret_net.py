...
class DualFigretNetWork(nn.Module):
    def __init__(self, input_dim, output_dim, layer_num):
        """Initialize the FigretNetWork with the network structure.
        ...
        Args:
            ...
            layer_num: number of hidden layers
        """
        super(DualFigretNetWork, self).__init__()
        self.flatten = nn.Flatten()
        self.layers = []
        self.layers.append(nn.Linear(input_dim, 128))
        self.layers.append(nn.ReLU())
    ...
    def forward(self, x, paths_to_edges):
        """Forward the input data through the network.
        ...
        """
        x = self.flatten(x)
        logits = self.net(x) # (batch_size, num_edges)
        logits = torch.matmul(paths_to_edges, logits.t()).t() # (batch_size, num_paths)
        return logits
